...
When a conversation starts with a **read-only planning request** ("come up with a plan"), the Prismor scoped-agent synthesizes a deny list that locks the session to read-only tools. If the same session then shifts to **implementation**, Edit and Bash stay blocked because the synthesized rules are cached for the lifetime of the session.

## Root Cause

**File:** `prismor/runtime/scoped_agent.py` (installed package at `~/.local/pipx/venvs/immunity-agent/...`)

On the first `UserPromptSubmit` of a session, Prismor calls `synthesize_scoped_rules()` which analyzes the prompt intent and returns a `deny_tools` list. That result is cached to:

```
.prismor/scoped/<session-id>.json
```
...
```
python3 "/Users/anish/.local/pipx/venvs/immunity-agent/lib/python3.14/site-packages/prismor/runtime/cli.py" \
...
```bash
rm /Users/anish/Documents/immunity-agent/.prismor/scoped/*.json
```

Then start a new Claude Code session — Prismor will re-synthesize rules based on the new prompt intent.
...
|------|----------|--------|
| `prismor/runtime/scoped_agent.py` | `check_scoped_rules()` | Check `synthesized_at_turn`; if stale, return `None` to trigger re-synthesis |
| `prismor/runtime/cli.py` | hook-dispatch, scoped synthesis block | Write `turn` counter into cached JSON; pass current turn when calling `check_scoped_rules()` |

## Related

- Scoped session cache: `.prismor/scoped/<session-id>.json`
