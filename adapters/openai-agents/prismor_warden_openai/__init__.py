...
Wrap any agent tool with :func:`warden_guard` so every invocation is routed
through Warden's shared policy pipeline *before* the tool runs — the same
``warden.runtime.evaluate_tool_call`` that backs local coding-agent hooks. A
call that violates an enforce-mode policy (or the calling user's IAM profile)
raises :class:`WardenBlocked` and the underlying tool never executes.
...
Or guard a single tool / callable with :func:`warden_guard`. The wrapper is
...
from warden.principal import Subject, resolve_subject, use_subject
from warden.runtime import Decision, evaluate_tool_call

__all__ = ["warden_guard", "guard_agent", "use_subject", "WardenBlocked", "build_event"]

# event_type → the event field a string payload is matched against (mirrors
# warden.policy_engine._DEFAULT_FIELDS).
_TYPE_FIELD = {
    "shell": "command",
    "file_read": "path",
    "file_write": "path",
...
class WardenBlocked(Exception):
    """Raised when Warden denies a tool call. Carries the :class:`Decision`."""

    def __init__(self, reason: str, decision: Optional[Decision] = None) -> None:
        super().__init__(reason or "blocked by Prismor Warden policy")
...
def build_event(
    *,
    ...
    """Construct a canonical Warden event for a tool call (see warden/hooks.py)."""
...
def warden_guard(
    ...
    *,
    ...
    """Wrap ``tool`` so each call is evaluated by Warden before it runs.

    Args:
        tool: the callable (or OpenAI Agents SDK tool) to guard.
        subject: end-user the calls are attributed to — a ``Subject``, a
            ``WARDEN_SUBJECT``-style string (``"user:alice"``), or ``None`` to
        ...
        raise_on_block: raise :class:`WardenBlocked` on deny (default). If
    ...
    wrapper.__warden_guarded__ = True  # type: ignore[attr-defined]
    return wrapper
...
def _guard_function_tool(
    ...
    *,
    ...
    """
    if getattr(tool, "__warden_guarded__", False):
        return tool
    ...
    async def guarded(ctx: Any, input_str: Any) -> Any:
        ...
        if not decision.allow:  # honor the runtime decision (incl. org kill-switch / forced-enforce), not the app-passed mode
            ...
            return f"⛔ Prismor Warden blocked this tool call: {reason}"
        return await original(ctx, input_str)

    tool.on_invoke_tool = guarded
    tool.__warden_guarded__ = True
    return tool
...
def guard_agent(
    ...
    *,
    ...
    agent_obj.__warden_guarded_tools__ = guarded  # type: ignore[attr-defined]
    return agent_obj
