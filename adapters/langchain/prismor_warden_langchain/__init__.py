...
routed through ``warden.runtime.evaluate_tool_call`` before the tool runs, so a
...
* :func:`guard_tools` / :func:`warden_guard_tool` — wrap a ``BaseTool``'s
  implementation so a denied call never executes (hard enforcement). This is the
  reliable path and what you normally want.
* :class:`WardenCallbackHandler` — a ``BaseCallbackHandler`` for observability /
...
class WardenBlocked(Exception):
    """Raised when Warden denies a tool call (hard-stop mode)."""
...
def guard_tools(tools: Sequence[Any], **kwargs: Any) -> List[Any]:
    ...
    return [warden_guard_tool(t, **kwargs) for t in tools]
...
class WardenCallbackHandler(_BaseCB):  # type: ignore[misc]
    """Capture every tool call via ``on_tool_start`` and evaluate it.

    Add to any chain/agent run (``config={"callbacks": [WardenCallbackHandler()]}``)
    to record + evaluate tool calls even for tools you did not wrap. In enforce
    mode it raises :class:`WardenBlocked` from ``on_tool_start`` (which aborts the
