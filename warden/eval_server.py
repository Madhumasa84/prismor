"""warden/eval_server.py — HTTP evaluation endpoint for non-Python adapters.

Exposes the Warden policy pipeline as a local HTTP service so TypeScript,
...
from warden.principal import resolve_subject
...
class EvalHandler(BaseHTTPRequestHandler):
    ...
    def do_OPTIONS(self) -> None:  # noqa: N802
        ...
        self.send_header("Access-Control-Allow-Headers", "Content-Type, X-Warden-Subject")
        self.end_headers()
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        subject_str: Optional[str] = (
            self.headers.get("X-Warden-Subject")
            or body.get("subject")
        )
...
def run_eval_server(
    ...
    print(f"[warden] eval-server listening on http://{host}:{port}")
    print(f"[warden] workspace: {ws}")
    print(f"[warden] POST /v1/evaluate  →  tool call → Decision")
    print(f"[warden] GET  /health       →  liveness check")
    try:
        server.serve_forever()
    except KeyboardInterrupt:
        print("\n[warden] eval-server stopped.")


if __name__ == "__main__":
    import argparse as _ap
    _p = _ap.ArgumentParser(description="Prismor Warden HTTP evaluation server")
